...
self.addEventListener("activate",event=>{
 event.waitUntil((async()=>{
  ...
  const oldEdan=keys.filter(k=>k!==CACHE&&k.startsWith("edan-tabasco-"));
  // Antes de borrar una versión anterior, rescatar jsPDF/QRCode ya almacenados.
  for(const url of OPTIONAL){
   if(await cache.match(url))continue;
   for(const oldName of oldEdan){
    try{
     const oldCache=await caches.open(oldName);
     const hit=await oldCache.match(url);
     if(hit){await cache.put(url,hit.clone());break}
    }catch(e){}
   }
  }
  // Solo después de migrar recursos opcionales se eliminan las cachés antiguas.
  await Promise.all(oldEdan.map(k=>caches.delete(k)));
